...
module VotesHelper
  ...
  def like_link(resource)
    link_to resource.have_vote?(current_user) ? 'Unvote' : 'Like',
            like_votable_path(resource),
            method: :patch,
            class: 'vote-link like',
            data: { type: :json },
            remote: true
  end
  ...
  def dislike_link(resource)
    if resource.have_vote?(current_user)
      return link_to 'Dislike',
                     dislike_votable_path(resource),
                     method: :patch,
                     class: 'vote-link dislike hidden',
                     data: { type: :json },
                     remote: true
    end
